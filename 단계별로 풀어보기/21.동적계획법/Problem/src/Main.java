...
public class Main {
    ...
    private static int dp2579(int N, int cnt) {
        if(cnt == 2){
            return 0;
        }
        if(N <= 2)
            return score[N] = arr[N];
        if(score[N] == 0) {
            score[N] = Math.max(dp2579(N-1, cnt+1), dp2579(N - 2, 0)) + arr[N];

            return score[N];
        }

        return score[N];// = Math.max(score[N], Math.max(dp2579(N-1, cnt+1), dp2579(N - 2, 0)) + arr[N]);
    }
